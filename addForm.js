...
// bugfixing: cannot find 'team'; breaks drawDOM
function drawDOM() {
    let teamDiv = document.getElementById('teams');
    clearElement(teamDiv);
    // Encounter Error: Failed to execute 'appendChild' on 'Node': parameter 1 is not of type 'Node'.
    for (team of teams) {
...
function creatTeamTable(team) {
    let table = document.createElement('table');
    table.setAttribute('class', 'table table-dark table-striped');
    let row = table.insertRow(0);
    let nameColumn = document.createElement('th');
    let positionColumn = document.createElement('th');
    nameColumn.innerHTML = 'Familial';
    positionColumn.innerHTML = 'Particular';
